...
public class Product implements Serializable{
	...
	public void makeDoc() {
		try {
			if (this.doc == null) {
				ENV.setProxy();
				this.doc = Jsoup.connect(url).timeout(3000)
	...
	/**
	 * This method will get the image in jpg format from the product page of
	 * Morrisons. Execution will take time as it will query online.
	 * 
	...
	public BufferedImage getProductImage() {
		...
		/*
		 * File outputfile = new File("saved.jpg"); try { ImageIO.write(image,
		 * ENV.IMAGE_WRITE_OPTION, outputfile); } catch (IOException e) {
	...
	public ProductPrice getProductPrice() {
		...
		Element currencyElement = priceElement.select("meta[content=GBP]")
				.first();
		productPrice.currency = currencyElement.attr("content");

		Element pricePerWeightElement = priceElement.select("p.pricePerWeight")
				.first();
